...
- Require correct top-1 labels in addition to reference/mixed equality because
  the selected ten-image set has an explicit balanced ground truth.
...
- Tasks 5-7: graph bundles, HOST/FSIM runtime, CLI and user documentation
- Required focused tests: `test_graph_artifacts.py`,
  `test_host_deployment.py`
- Required result: LLVM/C reference and mixed bundles execute ten exact,
  correctly labelled comparisons with positive FSIM activity

### Checkpoint 3: TSIM and repository integration

- Tasks 8-9: TSIM contract, extractor and aggregate validation automation
...
|---|---|---|
| VWW MobileNet contains depthwise convolutions unsupported by VTA | High | Assert the observed 12 VTA regions and host fallback set; never require all convolutions to offload |
...
- No changes to TVM, VTA compiler internals, existing ResNet applications, or
