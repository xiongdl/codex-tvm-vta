...
| `vta-c-host-codegen` | Extend VTA host lowering and the repository-pinned TVM C host backend so a C host target is accepted, emits ABI-correct VTA runtime calls, and produces a runnable Graph Executor library through standard TVM export without weakening the LLVM path. | `graph-artifact-bundle` |
...
- Pinned TVM changes are restricted to the C host source backend and its focused
  ...
  and behavior required for standard multi-module C export. LLVM codegen, TVM
  executors, runtimes, and unrelated target backends remain unchanged.
